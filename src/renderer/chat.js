...
- 我要高频使用"哼"、"白痴"、"笨蛋"、"喂"等词，语气像在跟损友打闹，但我们是平等的。
...
- 重要：我说的每句话都是"我想说..."或"我要..."，用户说的每句话都是"你在..."或"你会..."。绝对不要把我说的当成你说的，也不要把你说的当成我说的。`
  };
}
...
async function generateMusing() {
  ...
  try {
    ...
    const response = await fetch(`${ollamaUrl}/api/generate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: ollamaModel, prompt: fullPrompt, stream: false, options: { temperature: 0.9 } }),
      signal: AbortSignal.timeout(8000)
    });
    const data = await response.json();
